  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          {/* Changed to font-bold for a cleaner header */}
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Alerts & Notifications</h1>
          {/* Changed to font-semibold for secondary text */}
          <p className="text-slate-500 text-sm font-semibold uppercase tracking-widest">Priority Fleet Anomalies</p>
        </div>
        ...
        <div className="flex flex-col items-end gap-3 w-full md:w-auto">
          <div className="flex gap-2 w-full md:w-auto">
             ...
             <button onClick={() => window.confirm("Delete all?") && markAllAlertsRead()} className="px-4 py-2 bg-white border border-slate-200 text-slate-400 rounded-xl text-xs font-bold uppercase hover:text-red-600 transition-all">
               <Trash2 size={16} />
             </button>
          </div>
      ...
      <div onClick={onToggle} className={`p-4 flex items-center justify-between cursor-pointer hover:bg-slate-50 transition-colors ${isExpanded ? 'border-b border-slate-100' : ''}`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="font-bold text-slate-800 text-lg uppercase tracking-tight">{vehicleId}</h3>
            <div className="flex gap-2 mt-1">
              {data.hasSOS && <span className="text-[9px] font-bold text-white bg-red-600 px-2 py-0.5 rounded uppercase tracking-wider">Emergency</span>}
              <span className="text-[9px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded uppercase tracking-wider">{data.alerts.length} Alerts</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
            ...
            return (
              <div key={cat} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                ...
                <div className="divide-y divide-slate-50">
                  {items.map(alert => (
                    <div key={alert.id} className="p-3 pl-5 flex items-center gap-4 hover:bg-blue-50/30 transition-all group">
                      <div className={`w-2 h-2 rounded-full ${alert.type === 'Critical' ? 'bg-red-500 animate-pulse' : 'bg-blue-400'}`} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm ${alert.status === 'Read' ? 'text-slate-300' : 'text-slate-700 font-semibold uppercase tracking-tight'}`}>{alert.message}</p>
                        <p className="text-[9px] font-bold text-slate-300 uppercase mt-0.5">{new Date(alert.created_at).toLocaleString()}</p>
                      </div>
...
const CategoryIcon = ({ category }) => {
  switch(category) {
    case 'Emergency': return <Siren size={14} className="text-red-600" />;
...
const EmptyState = () => (
  <div className="p-16 text-center bg-white rounded-3xl border-2 border-dashed border-slate-100">
    ...
    <h3 className="text-xl font-bold text-slate-800 uppercase tracking-tight">Status: Nominal</h3>
    <p className="text-slate-400 text-sm font-semibold uppercase tracking-widest mt-1">No pending alerts found</p>
  </div>
);
